...
class TMDbClient {
    ...
    getClient() {
        if (!this.client) {
            // 验证API密钥
            const apiKey = (0, config_1.validateApiKey)();
            // 创建Axios实例
            this.client = axios_1.default.create({
                baseURL: 'https://api.themoviedb.org/3',
                params: {
                    api_key: apiKey,
                    language: 'en-US' // 默认使用中文
                }
            });
        }
        return this.client;
     ...
     * @returns 搜索结果
     */
    async searchTvShowByTitle(title) {
        try {
            const response = await this.getClient().get('/search/tv', {
                params: {
                    query: title,
     ...
     * @returns 相似剧集列表
     */
    async getSimilarTvShows(tvId, limit = 10) {
        try {
     ...
     * @returns 观看渠道信息
     */
    async getTvShowWatchProviders(tvId, countryCode = 'US') {
        try {
     ...
     * @returns 用户评论列表
     */
    async getTvShowReviews(tvId, page = 1) {
        try {
...
// 导出单例实例
exports.tmdbClient = new TMDbClient();
// 导出类型以供其他文件使用
