...
const discoverShowsTool_1 = require("../tools/discoverShowsTool");
...
describe('discoverShows Tool', () => {
    beforeEach(() => {
        ...
        mapGenreToId.mockImplementation((genre) => {
            if (genre === '喜剧')
                return 35;
            if (genre === '科幻')
                return 10765;
    ...
    it('应该使用默认参数正确调用API', async () => {
        ...
        tmdbClient_1.tmdbClient.discoverTvShows.mockResolvedValue(mockResponse);
        // 执行测试
        const result = await (0, discoverShowsTool_1.discoverShows)({});
        // 验证调用
        expect(tmdbClient_1.tmdbClient.discoverTvShows).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('应该正确处理类型参数', async () => {
        ...
        tmdbClient_1.tmdbClient.discoverTvShows.mockResolvedValue(mockResponse);
        // 执行测试
        await (0, discoverShowsTool_1.discoverShows)({
            with_genres: ['喜剧', '科幻']
        });
        // 验证调用
        expect(tmdbClient_1.tmdbClient.discoverTvShows).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('应该抛出错误当无法识别所有类型', async () => {
        const { mapGenreToId } = require('../utils/genreMap');
        mapGenreToId.mockReturnValue(null);
        // 执行测试并验证错误
        await expect((0, discoverShowsTool_1.discoverShows)({
            with_genres: ['未知类型']
    ...
    it('应该使用findShowsByPersonName查找演员参与的作品', async () => {
        ...
        tmdbClient_1.tmdbClient.searchPerson.mockResolvedValue(mockPersonResponse);
        tmdbClient_1.tmdbClient.getPersonTvCredits.mockResolvedValue(mockCreditsResponse);
        // 执行测试
        const result = await (0, discoverShowsTool_1.findShowsByPersonName)('布莱恩·科兰斯顿');
        // 验证调用
        expect(tmdbClient_1.tmdbClient.searchPerson).toHaveBeenCalledWith('布莱恩·科兰斯顿');
    ...
    it('应该使用getRecommendationsByActor获取演员推荐', async () => {
        ...
        tmdbClient_1.tmdbClient.getPersonTvCredits.mockResolvedValue(mockCreditsResponse);
        // 执行测试
        const result = await (0, discoverShowsTool_1.getRecommendationsByActor)('布莱恩·科兰斯顿', 5);
        // 验证调用
        expect(tmdbClient_1.tmdbClient.searchPerson).toHaveBeenCalledWith('布莱恩·科兰斯顿');
        expect(tmdbClient_1.tmdbClient.getPersonTvCredits).toHaveBeenCalledWith(123);
        // 验证结果
    ...
    it('findShowsByPersonName应该在找不到人物时抛出错误', async () => {
        tmdbClient_1.tmdbClient.searchPerson.mockResolvedValue({ results: [] });
        // 执行测试并验证错误
        await expect((0, discoverShowsTool_1.findShowsByPersonName)('未知演员')).rejects.toThrow('未找到人物');
    });
    it('getRecommendationsByActor应该在找不到演员时抛出错误', async () => {
        tmdbClient_1.tmdbClient.searchPerson.mockResolvedValue({ results: [] });
        // 执行测试并验证错误
        await expect((0, discoverShowsTool_1.getRecommendationsByActor)('未知演员')).rejects.toThrow('未找到演员');
    });
    it('应该正确处理年份参数并过滤结果', async () => {
        ...
        expect(console.log).toHaveBeenCalledWith(expect.stringContaining('无日期剧集') && expect.stringContaining('缺少首播日期'));
        expect(console.log).toHaveBeenCalledWith(expect.stringContaining('2023剧集') && expect.stringContaining('不匹配'));
    ...
    it('应该正确处理关键词参数', async () => {
        ...
        tmdbClient_1.tmdbClient.searchKeyword.mockResolvedValue(mockKeywordResponse);
        // 模拟API响应
        ...
        tmdbClient_1.tmdbClient.discoverTvShows.mockResolvedValue(mockDiscoverResponse);
        // 执行测试
        await (0, discoverShowsTool_1.discoverShows)({
            with_keywords: ['太空', '机器人']
        });
        // 验证关键词搜索被正确调用
        expect(tmdbClient_1.tmdbClient.searchKeyword).toHaveBeenCalledWith('太空');
        expect(tmdbClient_1.tmdbClient.searchKeyword).toHaveBeenCalledWith('机器人');
    ...
    it('应该抛出错误当无法识别所有关键词', async () => {
        // 模拟关键词搜索返回空结果
        tmdbClient_1.tmdbClient.searchKeyword.mockResolvedValue({ results: [] });
        // 执行测试并验证错误
        await expect((0, discoverShowsTool_1.discoverShows)({
            with_keywords: ['未知关键词']
    ...
    it('应该处理API调用错误', async () => {
        // 模拟API调用错误
        tmdbClient_1.tmdbClient.discoverTvShows.mockRejectedValue(new Error('API错误'));
        // 执行测试并验证错误被正确传递
        await expect((0, discoverShowsTool_1.discoverShows)({})).rejects.toThrow('API错误');
    });
    it('应该处理空结果集', async () => {
        const mockResponse = {
            page: 1,
    ...
    it('findPersonId应该返回null当未找到人物', async () => {
        tmdbClient_1.tmdbClient.searchPerson.mockResolvedValue({ results: [] });
        const result = await (0, discoverShowsTool_1.findPersonId)('未知人物');
        expect(result).toBeNull();
    });
    it('findPersonId应该返回null当API调用失败', async () => {
        tmdbClient_1.tmdbClient.searchPerson.mockRejectedValue(new Error('API错误'));
        const result = await (0, discoverShowsTool_1.findPersonId)('人物名');
        expect(result).toBeNull();
    });
    it('应该测试演员转换为ID的逻辑', async () => {
        ...
        tmdbClient_1.tmdbClient.getPersonTvCredits.mockResolvedValue(mockCreditsResponse);
        // 通过调用公开API来间接测试内部函数
        await (0, discoverShowsTool_1.getRecommendationsByActor)('布莱恩·科兰斯顿', 10);
        // 测试找不到演员的情况
        tmdbClient_1.tmdbClient.searchPerson.mockResolvedValue({ results: [] });
        await expect((0, discoverShowsTool_1.getRecommendationsByActor)('未知演员')).rejects.toThrow('未找到演员');
    });
    it('应该处理API错误情况', async () => {
        ...
        tmdbClient_1.tmdbClient.searchKeyword.mockRejectedValue(new Error('API错误'));
    ...
    it('应该测试getPersonTvCredits函数处理API错误的情况', async () => {
        // 模拟API调用错误
        tmdbClient_1.tmdbClient.getPersonTvCredits.mockRejectedValue(new Error('API错误'));
        // 执行测试并验证错误被正确传递
        await expect((0, discoverShowsTool_1.getPersonTvCredits)(123)).rejects.toThrow('获取人物电视作品失败');
    });
    ...
    it('应该正确测试 API 错误处理（演员相关）', async () => {
        // 模拟演员搜索 API 抛出错误
        tmdbClient_1.tmdbClient.searchPerson.mockRejectedValue(new Error('API 错误'));
        // 应该返回 null 而不是抛出错误
        const result = await (0, discoverShowsTool_1.findPersonId)('演员名');
        expect(result).toBeNull();
    });
    it('应该处理空演员数组', async () => {
        // 测试 findShowsByPersonName 使用空字符串参数，这会间接测试内部的演员转换函数
        tmdbClient_1.tmdbClient.searchPerson.mockResolvedValue({
            results: []
        });
        await expect((0, discoverShowsTool_1.findShowsByPersonName)('')).rejects.toThrow('未找到人物');
    });
    it('应该测试关键词处理的更多场景', async () => {
        ...
        expect(tmdbClient_1.tmdbClient.discoverTvShows).toHaveBeenCalledWith(expect.not.objectContaining({
            with_keywords: expect.anything()
        }));
        // 2. 关键词 ID 转换（一个关键词成功，一个失败）
        tmdbClient_1.tmdbClient.searchKeyword
    ...
    it('应该实现 110 行的覆盖', async () => {
        ...
        tmdbClient_1.tmdbClient.getPersonTvCredits.mockResolvedValue({
            cast: [
                {
                    id: 1,
                    name: '测试剧集',
        ...
        await (0, discoverShowsTool_1.getRecommendationsByActor)('测试演员', 1);
        // 验证相关的API调用
        expect(tmdbClient_1.tmdbClient.searchPerson).toHaveBeenCalledWith('测试演员');
        expect(tmdbClient_1.tmdbClient.getPersonTvCredits).toHaveBeenCalledWith(123);
    });
});
