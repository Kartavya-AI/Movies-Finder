...
describe('TMDbClient', () => {
    ...
    beforeEach(() => {
        ...
        axios_1.default.create.mockReturnValue(mockAxiosClient);
        // 确保client属性是null，这样会触发懒加载
        Object.defineProperty(tmdbClient_1.tmdbClient, 'client', {
            value: null,
            writable: true
        });
        // 通过调用一个方法触发懒加载
        tmdbClient_1.tmdbClient.testConnection();
        // 现在可以获取get方法
    ...
    it('测试连接应该返回正确结果', async () => {
        ...
        expect(await tmdbClient_1.tmdbClient.testConnection()).toBe(true);
        // 失败情况
        mockAxiosGet.mockRejectedValueOnce(new Error('连接失败'));
        expect(await tmdbClient_1.tmdbClient.testConnection()).toBe(false);
    });
    ...
    it('getConfiguration应该处理错误', async () => {
        ...
        await expect(tmdbClient_1.tmdbClient.getConfiguration()).rejects.toThrow('获取TMDb配置失败');
    });
    it('searchTvShow应该正确调用API', async () => {
        const mockResponse = { data: { results: [] } };
        mockAxiosGet.mockResolvedValueOnce(mockResponse);
        const result = await tmdbClient_1.tmdbClient.searchTvShow('测试剧集');
    ...
    it('searchTvShow应该处理错误', async () => {
        mockAxiosGet.mockRejectedValueOnce(new Error('API错误'));
        await expect(tmdbClient_1.tmdbClient.searchTvShow('测试剧集')).rejects.toThrow('查询TV节目');
    });
    it('getRecommendationsByGenre应该正确调用API', async () => {
        const mockResponse = { data: { results: [{}, {}, {}] } };
        mockAxiosGet.mockResolvedValueOnce(mockResponse);
        const result = await tmdbClient_1.tmdbClient.getRecommendationsByGenre(35, 2);
    ...
    it('getRecommendationsByGenre应该处理错误', async () => {
        mockAxiosGet.mockRejectedValueOnce(new Error('API错误'));
        await expect(tmdbClient_1.tmdbClient.getRecommendationsByGenre(35)).rejects.toThrow('获取类型ID 35 的推荐失败');
    });
    ...
    it('getTvGenres应该处理错误', async () => {
        mockAxiosGet.mockRejectedValueOnce(new Error('API错误'));
        await expect(tmdbClient_1.tmdbClient.getTvGenres()).rejects.toThrow('获取TV类型列表失败');
    });
    ...
    it('searchTvShowByTitle应该处理错误', async () => {
        mockAxiosGet.mockRejectedValueOnce(new Error('API错误'));
        await expect(tmdbClient_1.tmdbClient.searchTvShowByTitle('测试剧集')).rejects.toThrow('搜索剧集');
    });
    it('getSimilarTvShows应该正确调用API', async () => {
        const mockResponse = { data: { results: [{}, {}, {}] } };
        mockAxiosGet.mockResolvedValueOnce(mockResponse);
        const result = await tmdbClient_1.tmdbClient.getSimilarTvShows(123, 2);
    ...
    it('getSimilarTvShows应该处理错误', async () => {
        mockAxiosGet.mockRejectedValueOnce(new Error('API错误'));
        await expect(tmdbClient_1.tmdbClient.getSimilarTvShows(123)).rejects.toThrow('获取剧集ID 123 的相似剧集失败');
    });
    it('getTvShowWatchProviders应该正确调用API', async () => {
        ...
        mockAxiosGet.mockResolvedValueOnce(mockResponse);
        // 测试默认国家/地区代码
        const result1 = await tmdbClient_1.tmdbClient.getTvShowWatchProviders(123);
        ...
        // 测试指定国家/地区代码
        mockAxiosGet.mockResolvedValueOnce(mockResponse);
        const result2 = await tmdbClient_1.tmdbClient.getTvShowWatchProviders(123, 'CN');
    ...
    it('getTvShowWatchProviders应该处理国家/地区不存在的情况', async () => {
        ...
        mockAxiosGet.mockResolvedValueOnce(mockResponse);
        const result = await tmdbClient_1.tmdbClient.getTvShowWatchProviders(123, 'XYZ');
        expect(result.results).toEqual({});
    });
    it('getTvShowWatchProviders应该处理错误', async () => {
        mockAxiosGet.mockRejectedValueOnce(new Error('API错误'));
        await expect(tmdbClient_1.tmdbClient.getTvShowWatchProviders(123)).rejects.toThrow('获取剧集ID 123 的观看渠道失败');
    });
    it('discoverTvShows应该正确调用API', async () => {
        const mockResponse = { data: { results: [] } };
        mockAxiosGet.mockResolvedValueOnce(mockResponse);
        // 使用默认参数
        const result1 = await tmdbClient_1.tmdbClient.discoverTvShows();
        expect(mockAxiosGet).toHaveBeenCalledWith('/discover/tv', {
            params: { page: 1 }
        });
        // 使用自定义参数
        mockAxiosGet.mockResolvedValueOnce(mockResponse);
        const result2 = await tmdbClient_1.tmdbClient.discoverTvShows({
            with_genres: '35,18',
    ...
    it('discoverTvShows应该处理错误', async () => {
        mockAxiosGet.mockRejectedValueOnce(new Error('API错误'));
        await expect(tmdbClient_1.tmdbClient.discoverTvShows()).rejects.toThrow('高级剧集发现失败');
    });
    it('searchPerson应该正确调用API', async () => {
        const mockResponse = { data: { results: [] } };
        mockAxiosGet.mockResolvedValueOnce(mockResponse);
        const result = await tmdbClient_1.tmdbClient.searchPerson('演员名');
    ...
    it('searchPerson应该处理错误', async () => {
        mockAxiosGet.mockRejectedValueOnce(new Error('API错误'));
        await expect(tmdbClient_1.tmdbClient.searchPerson('演员名')).rejects.toThrow('搜索人物');
    });
    it('searchKeyword应该正确调用API', async () => {
        const mockResponse = { data: { results: [] } };
        mockAxiosGet.mockResolvedValueOnce(mockResponse);
        const result = await tmdbClient_1.tmdbClient.searchKeyword('关键词');
    ...
    it('searchKeyword应该处理错误', async () => {
        mockAxiosGet.mockRejectedValueOnce(new Error('API错误'));
        await expect(tmdbClient_1.tmdbClient.searchKeyword('关键词')).rejects.toThrow('搜索关键词');
    });
    ...
    it('getNetworks应该处理错误', async () => {
        mockAxiosGet.mockRejectedValueOnce(new Error('API错误'));
        await expect(tmdbClient_1.tmdbClient.getNetworks()).rejects.toThrow('获取电视网络列表失败');
    });
});
