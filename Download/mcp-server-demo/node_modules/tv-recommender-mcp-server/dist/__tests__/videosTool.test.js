...
const videosTool_1 = require("../tools/videosTool");
...
describe('videosTool', () => {
    ...
    it('应该成功获取剧集视频', async () => {
        ...
        // 设置模拟函数的返回值
        tmdbClient_1.tmdbClient.searchTvShowByTitle.mockResolvedValue(mockSearchResults);
        tmdbClient_1.tmdbClient.getTvShowVideos.mockResolvedValue(mockVideosData);
        // 执行函数
        const result = await (0, videosTool_1.getShowVideos)({ show_title: '权力的游戏' });
        // 验证模拟函数是否被正确调用
        expect(tmdbClient_1.tmdbClient.searchTvShowByTitle).toHaveBeenCalledWith('权力的游戏');
    ...
    it('应该处理搜索不到剧集的情况', async () => {
        // 模拟空搜索结果
        tmdbClient_1.tmdbClient.searchTvShowByTitle.mockResolvedValue({ results: [] });
        // 执行并确认抛出正确的错误
        await expect((0, videosTool_1.getShowVideos)({ show_title: '不存在的剧集' }))
            .rejects
            .toThrow('未找到名为"不存在的剧集"的剧集');
    });
    it('应该处理API错误情况', async () => {
        // 模拟API错误
        tmdbClient_1.tmdbClient.searchTvShowByTitle.mockRejectedValue(new Error('网络错误'));
        // 执行并确认抛出正确的错误
        await expect((0, videosTool_1.getShowVideos)({ show_title: '权力的游戏' }))
            .rejects
    ...
    it('应该处理无视频的剧集', async () => {
        ...
        // 设置模拟函数返回值
        tmdbClient_1.tmdbClient.searchTvShowByTitle.mockResolvedValue(mockSearchResults);
        tmdbClient_1.tmdbClient.getTvShowVideos.mockResolvedValue(mockEmptyVideos);
        // 执行函数
        const result = await (0, videosTool_1.getShowVideos)({ show_title: '新剧集' });
        // 验证结果
    ...
    it('应该正确处理不同平台的视频URL', async () => {
        ...
        // 设置模拟函数返回值
        tmdbClient_1.tmdbClient.searchTvShowByTitle.mockResolvedValue(mockSearchResults);
        tmdbClient_1.tmdbClient.getTvShowVideos.mockResolvedValue(mockMultiPlatformVideos);
        // 执行函数
        const result = await (0, videosTool_1.getShowVideos)({ show_title: '测试剧集' });
        // 验证不同平台URL的生成
    ...
    it('应该根据类型和官方性质对视频进行排序', async () => {
        ...
        // 设置模拟函数返回值
        tmdbClient_1.tmdbClient.searchTvShowByTitle.mockResolvedValue(mockSearchResults);
        tmdbClient_1.tmdbClient.getTvShowVideos.mockResolvedValue(mockSortingVideos);
        // 执行函数
        const result = await (0, videosTool_1.getShowVideos)({ show_title: '排序测试' });
