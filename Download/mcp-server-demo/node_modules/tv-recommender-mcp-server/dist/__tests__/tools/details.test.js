...
const details_1 = require("../../tools/details");
// 模拟 axios
jest.mock('axios');
...
describe('getShowDetails Tool', () => {
    ...
    it('应该正确返回剧集详情', async () => {
        ...
        const result = await (0, details_1.getShowDetails)({ show_title: '西部世界' });
        ...
        expect(mockedAxios.get).toHaveBeenNthCalledWith(1, expect.stringContaining('/search/tv'), expect.anything());
        expect(mockedAxios.get).toHaveBeenNthCalledWith(2, expect.stringContaining('/tv/123'), expect.anything());
    });
    it('应该处理找不到剧集的情况', async () => {
        ...
        const result = await (0, details_1.getShowDetails)({ show_title: '一部不存在的剧' });
        expect(result).toBe('抱歉，未能找到您提供的剧集"一部不存在的剧"。');
        // 验证只调用了搜索 API
        expect(mockedAxios.get).toHaveBeenCalledTimes(1);
        expect(mockedAxios.get).toHaveBeenCalledWith(expect.stringContaining('/search/tv'), expect.anything());
    });
    ...
    it('应该正确映射剧集状态', async () => {
        ...
        for (const testCase of statusTestCases) {
            ...
            const result = await (0, details_1.getShowDetails)({ show_title: '测试剧集' });
    ...
    it('应该处理缺少首播日期的情况', async () => {
        ...
        const result = await (0, details_1.getShowDetails)({ show_title: '未公布日期的剧集' });
        // 验证结果，年份应该为null
