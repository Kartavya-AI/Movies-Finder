...
describe('getSimilarShows Tool', () => {
    ...
    it('应该成功获取相似剧集并返回格式化字符串', async () => {
        ...
        // 调用工具函数
        const result = await (0, recommendations_1.getSimilarShows)({ show_title: '怪奇物语' });
        // 验证输出
    ...
    it('当搜索不到剧集时应该返回错误提示', async () => {
        ...
        // 调用工具函数
        const result = await (0, recommendations_1.getSimilarShows)({ show_title: '不存在的剧集' });
        // 验证输出
    ...
    it('当没有相似剧集时应该返回相应提示', async () => {
        ...
        // 调用工具函数
        const result = await (0, recommendations_1.getSimilarShows)({ show_title: '独特剧集' });
        // 验证输出
        expect(result).toBe('抱歉，未能找到与"独特剧集"相似的剧集。');
    });
    it('当缺少参数时应该返回错误信息', async () => {
        // 调用工具函数，传入空参数
        const result = await (0, recommendations_1.getSimilarShows)({ show_title: '' });
        // 验证输出
