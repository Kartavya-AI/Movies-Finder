...
exports.getRecommendationsByGenre = getRecommendationsByGenre;
exports.getSimilarShows = getSimilarShows;
 ...
 * @returns 格式化的字符串
 */
function formatRecommendationsToString(genre, recommendations) {
 ...
 * @returns 格式化的字符串
 */
function formatSimilarShowsToString(originalTitle, recommendations) {
    if (!recommendations || recommendations.length === 0) {
 ...
 * @returns 推荐结果字符串
 */
async function getRecommendationsByGenre(params) {
    try {
        // 检查参数
        if (!params.genre) {
            throw new errorHandler_1.ApiError('缺少必要参数：genre', 400);
        }
        // 将用户输入的类型映射到TMDb类型ID
        const genreId = (0, genreMap_1.mapGenreToId)(params.genre);
        // 如果无法识别类型
        if (!genreId) {
            return `抱歉，无法识别您提供的类型"${params.genre}"，请尝试其他类型如"喜剧"、"科幻"等。`;
        }
        // 获取类型名称（使用中文）
        const genreName = (0, genreMap_1.getGenreNameById)(genreId) || params.genre;
        // 调用TMDb API获取推荐
        const response = await tmdbClient_1.tmdbClient.getRecommendationsByGenre(genreId);
        // 格式化结果
        ...
        return `获取推荐时发生错误: ${(0, errorHandler_1.formatErrorMessage)(error)}`;
    }
}
/**
 * 获取相似剧集推荐实现
 ...
 * @returns 推荐结果字符串
 */
async function getSimilarShows(params) {
    try {
        // 检查参数
        if (!params.show_title) {
            throw new errorHandler_1.ApiError('缺少必要参数：show_title', 400);
        }
        // 第一步：根据剧名搜索剧集
        const searchResponse = await tmdbClient_1.tmdbClient.searchTvShowByTitle(params.show_title);
        // 检查是否找到结果
        ...
        // 第二步：获取相似剧集
        const similarResponse = await tmdbClient_1.tmdbClient.getSimilarTvShows(showId);
        // 检查是否有相似剧集
        ...
        return `获取相似剧集时发生错误: ${(0, errorHandler_1.formatErrorMessage)(error)}`;
    }
}
